    ...
    public static class ModConfigAPI
    {
        ...
        private static bool CheckVersionCompatibility()
        {
            ...
            try
            {
                ...
                if (versionField != null && versionField.FieldType == typeof(int))
                {
                    ...
                    {
                        UnityEngine.Debug.LogError($"[{TAG}] 版本不匹配！API版本: {ModConfigVersion}, ModConfig版本: {modConfigVersion}");
                        return false;
                    }
                    UnityEngine.Debug.Log($"[{TAG}] 版本检查通过: {ModConfigVersion}");
                    versionChecked = true;
                    return true;
                }
                else
                {
                    ...
                    UnityEngine.Debug.LogWarning($"[{TAG}] 未找到版本信息字段，跳过版本检查");
            ...
            {
                UnityEngine.Debug.LogError($"[{TAG}] 版本检查失败: {ex.Message}");
        ...
        public static bool Initialize()
        {
            try
            {
                ...
                {
                    UnityEngine.Debug.LogWarning($"[{TAG}] ModConfig.ModBehaviour 类型未找到，ModConfig 可能未加载");
                    return false;
                }
                ...
                {
                    UnityEngine.Debug.LogWarning($"[{TAG}] ModConfig.OptionsManager_Mod 类型未找到");
                    return false;
                }
                // 检查版本兼容性
                // Check version compatibility
                ...
                {
                    UnityEngine.Debug.LogWarning($"[{TAG}] ModConfig version mismatch!!!");
                    return false;
                }
                // 检查必要的静态方法是否存在
                ...
                foreach (string methodName in requiredMethods)
                {
                    ...
                    if (method == null)
                    {
                        UnityEngine.Debug.LogError($"[{TAG}] 必要方法 {methodName} 未找到");
                        return false;
                    }
                }
                isInitialized = true;
                UnityEngine.Debug.Log($"[{TAG}] ModConfigAPI 初始化成功");
                return true;
            }
            catch (Exception ex)
            {
                UnityEngine.Debug.LogError($"[{TAG}] 初始化失败: {ex.Message}");
                return false;
            }
        }
        /// <summary>
            ...
            try
            {
                ...
                foreach (Assembly assembly in assemblies)
                {
                    try
                    {
                        ...
                        if (assembly.FullName.Contains("ModConfig")) UnityEngine.Debug.Log($"[{TAG}] 找到 ModConfig 相关程序集: {assembly.FullName}");
                        // 尝试在该程序集中查找类型
                        Type type = assembly.GetType(typeName);
                        if (type != null)
                        {
                            UnityEngine.Debug.Log($"[{TAG}] 在程序集 {assembly.FullName} 中找到类型 {typeName}");
                            return type;
                        }
                    }
                    catch
                ...
                // 记录所有已加载的程序集用于调试
                UnityEngine.Debug.LogWarning($"[{TAG}] 在所有程序集中未找到类型 {typeName}，已加载程序集数量: {assemblies.Length}");
                foreach (var assembly in assemblies.Where(a => a.FullName.Contains("ModConfig")))
                {
                    UnityEngine.Debug.Log($"[{TAG}] ModConfig 相关程序集: {assembly.FullName}");
                }
                return null;
            }
            catch (Exception ex)
            {
                UnityEngine.Debug.LogError($"[{TAG}] 程序集扫描失败: {ex.Message}");
                return null;
            }
        }
        /// <summary>
        ...
        public static bool SafeAddOnOptionsChangedDelegate(Action<string> action)
        {
            ...
            if (action == null)
            {
                UnityEngine.Debug.LogWarning($"[{TAG}] 不能添加空的事件委托");
                return false;
            }
            try
            {
                ...
                UnityEngine.Debug.Log($"[{TAG}] 成功添加选项变更事件委托");
                return true;
            }
            catch (Exception ex)
            {
                UnityEngine.Debug.LogError($"[{TAG}] 添加选项变更事件委托失败: {ex.Message}");
                return false;
            }
        }
        /// <summary>
        ...
        public static bool SafeRemoveOnOptionsChangedDelegate(Action<string> action)
        {
            ...
            if (action == null)
            {
                UnityEngine.Debug.LogWarning($"[{TAG}] 不能移除空的事件委托");
                return false;
            }
            try
            {
                ...
                UnityEngine.Debug.Log($"[{TAG}] 成功移除选项变更事件委托");
                return true;
            }
            catch (Exception ex)
            {
                UnityEngine.Debug.LogError($"[{TAG}] 移除选项变更事件委托失败: {ex.Message}");
                return false;
            }
        }
        /// <summary>
        ...
        public static bool SafeAddDropdownList(string modName, string key, string description, System.Collections.Generic.SortedDictionary<string, object> options, Type valueType, object defaultValue)
        {
            ...
            try
            {
                ...
                UnityEngine.Debug.Log($"[{TAG}] 成功添加下拉列表: {modName}.{key}");
                return true;
            }
            catch (Exception ex)
            {
                UnityEngine.Debug.LogError($"[{TAG}] 添加下拉列表失败 {modName}.{key}: {ex.Message}");
                return false;
            }
        }
        /// <summary>
        ...
        public static bool SafeAddInputWithSlider(string modName, string key, string description, Type valueType, object defaultValue, UnityEngine.Vector2? sliderRange = null)
        {
            ...
            try
            {
                ...
                UnityEngine.Debug.Log($"[{TAG}] 成功添加滑条输入框: {modName}.{key}");
                return true;
            }
            catch (Exception ex)
            {
                UnityEngine.Debug.LogError($"[{TAG}] 添加滑条输入框失败 {modName}.{key}: {ex.Message}");
                return false;
            }
        }
        /// <summary>
        ...
        public static bool SafeAddBoolDropdownList(string modName, string key, string description, bool defaultValue)
        {
            ...
            try
            {
                ...
                UnityEngine.Debug.Log($"[{TAG}] 成功添加布尔下拉列表: {modName}.{key}");
                return true;
            }
            catch (Exception ex)
            {
                UnityEngine.Debug.LogError($"[{TAG}] 添加布尔下拉列表失败 {modName}.{key}: {ex.Message}");
                return false;
            }
        }
        /// <summary>
        ...
        {
            key = $"{mod_name}_{key}";
            if (!Initialize()) return defaultValue;
            if (string.IsNullOrEmpty(key))
            {
                UnityEngine.Debug.LogWarning($"[{TAG}] 配置键不能为空");
                return defaultValue;
            }
            try
            {
                ...
                if (loadMethod == null)
                {
                    UnityEngine.Debug.LogError($"[{TAG}] 未找到 OptionsManager_Mod.Load 方法");
                    return defaultValue;
                }
                // 获取泛型方法
                ...
                UnityEngine.Debug.Log($"[{TAG}] 成功加载配置: {key} = {result}");
                return (T)result;
            }
            catch (Exception ex)
            {
                UnityEngine.Debug.LogError($"[{TAG}] 加载配置失败 {key}: {ex.Message}");
                return defaultValue;
            }
        }
        /// <summary>
        ...
        public static bool SafeSave<T>(string mod_name, string key, T value)
        {
            ...
            try
            {
                ...
                if (saveMethod == null)
                {
                    UnityEngine.Debug.LogError($"[{TAG}] 未找到 OptionsManager_Mod.Save 方法");
                    return false;
                }
                // 获取泛型方法
                MethodInfo genericSaveMethod = saveMethod.MakeGenericMethod(typeof(T));
                genericSaveMethod.Invoke(null, new object[] { key, value });
                UnityEngine.Debug.Log($"[{TAG}] 成功保存配置: {key} = {value}");
                return true;
            }
            catch (Exception ex)
            {
                UnityEngine.Debug.LogError($"[{TAG}] 保存配置失败 {key}: {ex.Message}");
                return false;
            }
        }
        /// <summary>
        ...
        {
            if (!Initialize()) return "ModConfig 未加载 | ModConfig not loaded";
            try
            {
